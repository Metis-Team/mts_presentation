 ...
 *      2: NUMBER - slideNumber. Numbers originate from the variable mts_presentation_topics (not needed if parameter 0 is true) (Optional, default: 1. First slide)
 *
 *  Returns:
 *      Nothing.
 *
...
if (!isNull _presenter) then {
    ...
    if !(_presenter isEqualTo _player) then {
        [
            [LLSTRING(mod_name), 1.2, [1, 1, 0, 1]],
            LLSTRING(presenter_slot_occupied_line_1),
            format [LLSTRING(presenter_slot_occupied_line_2), name _presenter],
            false
        ] call CBA_fnc_notify;
        breakOut "function";
    };
...
if (!_continue) then {
    if (_topic isEqualTo -1) then {
        ...
        _slideNumber =  (lbCurSel (_mainDisplay displayCtrl IDC_COMBOBOX)) + 1;
    } else {
        if (_topic < 0 && _slideNumber < 1) then {
    ...
    missionNamespace setVariable [QGVAR(currentSlideNumber), _slideNumber, true];

    if (GVAR(screens) isEqualTo []) then {
        [
            [LLSTRING(mod_name), 1.2, [1, 1, 0, 1]],
            LLSTRING(no_screen_to_display),
            false
        ] call CBA_fnc_notify;
